...
def go_callback_function(value):
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        node=dict(
            ...
            label=labels,
            color=rand_color.generate(count=len(labels))
        ),
        # Add links
        link=dict(
            source=source_list,
            target=target_list,
            value=[1 for _ in source_list],
            label=['no link label' for _ in source_list],
            color=rand_color.generate(count=len(source_list))
        ))])
